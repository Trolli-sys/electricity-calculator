...
FT_RATES = {
    ...
    # ใช้ค่าปัจจุบัน (เม.ย. 2568) เป็นค่าอ้างอิงไปก่อน หากยังไม่มีประกาศ
    (2025, 1): 0.3972, # ม.ค.-เม.ย. 68 (ตัวอย่าง - ใช้ค่าเดิมไปก่อน)
    (2025, 5): 0.3972, # พ.ค.-ส.ค. 68 (ตัวอย่าง - ใช้ค่าเดิมไปก่อน)
    (2025, 9): 0.3972, # ก.ย.-ธ.ค. 68 (ตัวอย่าง - ใช้ค่าเดิมไปก่อน)
}
...
# รายการวันหยุดปี 2024 (รวมวันหยุดสุดสัปดาห์และวันหยุดนักขัตฤกษ์)
HOLIDAYS_TOU_2024_STR = ["2024-01-01","2024-01-06","2024-01-07","2024-01-13","2024-01-14","2024-01-20","2024-01-21",
                         ...
                         "2024-10-05","2024-10-06","2024-10-12","2024-10-13",
...
HOLIDAYS_TOU_2024_STR = sorted(list(set(HOLIDAYS_TOU_2024_STR)))

# วันหยุดปี 2025 (รวมวันหยุดสุดสัปดาห์และวันหยุดนักขัตฤกษ์ - ควรตรวจสอบความถูกต้องอีกครั้ง)
    ...
    try:
        ...
        # 4. จัดการรูปแบบ Comma-Separated
            ...
            for s in potential_dates:
                ...
                elif '-' in s: # มักจะเป็น<y_bin_46>-MM-DD
                    st.write("✔️ ตรวจพบรูปแบบวันที่อาจเป็น<y_bin_46>-MM-DD ('-' separator)")
                    dayfirst_guess = False #<y_bin_46>-MM-DD ไม่ใช่ dayfirst
                    guessed = True
                    break
            if not guessed and not potential_dates.empty:
                 st.write("⚠️ ไม่สามารถเดารูปแบบวันที่ (DD/MM หรือ<y_bin_46>-MM) ได้ชัดเจน, ใช้ค่าเริ่มต้น dayfirst=False")
...
def classify_tou_period(dt_obj):
    ...
    # แก้ไข: ใช้รายการวันหยุดจาก HOLIDAYS_TOU_DATA ที่แปลงเป็น set แล้ว
...
รองรับไฟล์ .txt หรือ .csv ที่มีรูปแบบทั่วไป
        ...
        with st.spinner("กำลังคำนวณค่าไฟฟ้า..."):
            try:
                ...
                if df_filtered.empty:
                    ...
                    # --- 5. สร้างข้อความผลลัพธ์ (ปรับปรุงการจัดรูปแบบ) ---
                    if bill_details.get("error"):
                        ...
                        if ev_enabled: output += ev_details_for_output
                        ...
                        # --- รายละเอียดค่าใช้จ่าย (จัดรูปแบบใหม่) ---
                        label_width = 35 # กำหนดความกว้างของคอลัมน์ซ้าย (ป้ายกำกับ)
                        value_width = 15 # กำหนดความกว้างของคอลัมน์ขวา (ตัวเลข)
                        separator_len = label_width + value_width + 7 # ความยาวเส้นคั่นโดยประมาณ

                        output += "-" * separator_len + "\n" # เส้นคั่น
                        output += f"{'ค่าพลังงานไฟฟ้า':<{label_width}}: {bill_details['base_energy_cost']:>{value_width},.2f} บาท\n"
                        output += f"{'ค่าบริการรายเดือน':<{label_width}}: {bill_details['service_charge']:>{value_width},.2f} บาท\n"
                        # สร้าง label สำหรับ Ft ก่อน เพื่อจัดการความยาว
                        ft_rate_for_period = bill_details['applicable_ft_rate']
                        ft_year_used = bill_details['data_year']
                        ft_label = f"ค่า Ft (@{ft_rate_for_period:.4f} บาท/หน่วย, ปี {ft_year_used})"
                        output += f"{ft_label:<{label_width}}: {bill_details['ft_cost']:>{value_width},.2f} บาท\n"

                        # ยอดรวมและ VAT (จัดรูปแบบใหม่)
                        output += "-" * separator_len + "\n" # เส้นคั่น
                        output += f"{'ยอดรวมก่อน VAT':<{label_width}}: {bill_details['total_before_vat']:>{value_width},.2f} บาท\n"
                        vat_label = f"ภาษีมูลค่าเพิ่ม ({VAT_RATE*100:.0f}%)"
                        output += f"{vat_label:<{label_width}}: {bill_details['vat_amount']:>{value_width},.2f} บาท\n"
                        output += "=" * separator_len + "\n" # เส้นคั่นคู่
                        # ทำให้บรรทัดสุดท้ายเด่นขึ้น
                        final_bill_label = "**ยอดค่าไฟฟ้าสุทธิ**"
                        # จัดแนว label ชิดซ้าย, ตัวเลขชิดขวา
                        output += f"{final_bill_label:<{label_width}}: {bill_details['final_bill']:>{value_width},.2f} **บาท**\n"
                        output += "=" * separator_len + "\n" # เส้นคั่นคู่
                        # --- สิ้นสุดรายละเอียดค่าใช้จ่าย ---

                        # --- (ส่วนหมายเหตุเพิ่มเติม เหมือนเดิม) ---
                        output += f"\nหมายเหตุ:\n"
...
if st.session_state.full_dataframe is not None and not st.session_state.full_dataframe.empty:
    st.markdown("---")
    # Checkbox for showing data preview
    ...
    # Checkbox for showing the graph for the selected period
    if st.checkbox("📊 แสดงกราฟ Load Profile (ตามช่วงเวลาที่เลือก)", key="show_filtered_graph"):
        st.subheader("กราฟ Load Profile (kW Demand) - ตามช่วงเวลาที่เลือก")
        try:
            # ดึงค่าปีและเดือนที่เลือกจาก selectbox ผ่าน session state
            selected_year_graph = st.session_state.get("year", "ทั้งปี") # ใช้ .get เผื่อยังไม่มีค่า
            selected_month_graph = st.session_state.get("month", "ทั้งเดือน")

            # เริ่มต้นด้วยการคัดลอกข้อมูลทั้งหมด
            df_to_plot = st.session_state.full_dataframe.copy()
            graph_period_label = "ข้อมูลทั้งหมดในไฟล์" # ข้อความเริ่มต้น

            # --- ทำการกรองข้อมูลเหมือนตอนคำนวณ ---
            # กรองตามปี
            if selected_year_graph != "ทั้งปี":
                try:
                    year_int_graph = int(selected_year_graph)
                    df_to_plot = df_to_plot[df_to_plot['DateTime'].dt.year == year_int_graph]
                    graph_period_label = f"ปี {year_int_graph}" # อัปเดตข้อความ
                except ValueError:
                    st.error(f"ปีที่เลือกสำหรับกราฟ ('{selected_year_graph}') ไม่ถูกต้อง")
                    df_to_plot = pd.DataFrame() # ทำให้ DataFrame ว่างเปล่าเพื่อหยุดพล็อต

            # กรองตามเดือน
            if selected_month_graph != "ทั้งเดือน" and not df_to_plot.empty: # กรองเดือนต่อเมื่อมีข้อมูลและเลือกเดือนไว้
                month_num_graph = None
                for num, name in MONTH_NAMES_TH.items():
                    if name == selected_month_graph:
                        month_num_graph = num
                        break
                if month_num_graph:
                    df_to_plot = df_to_plot[df_to_plot['DateTime'].dt.month == month_num_graph]
                    # อัปเดตข้อความแสดงช่วงเวลา
                    if graph_period_label.startswith("ปี"):
                        graph_period_label = f"{selected_month_graph} {graph_period_label}"
                    else: # กรณีเลือก "ทั้งปี" แต่เลือกเดือน
                        graph_period_label = f"เดือน{selected_month_graph} (ทุกปีในข้อมูล)"
                else:
                     # ถ้าชื่อเดือนไม่ถูกต้อง จะแสดงข้อมูลที่กรองตามปี (ถ้ามี)
                     st.warning(f"ไม่พบเลขเดือนสำหรับกราฟ: '{selected_month_graph}'.")

            # --- สิ้นสุดการกรอง ---

            # ตรวจสอบว่ามีข้อมูลเหลือหลังจากการกรองหรือไม่
            if not df_to_plot.empty:
                # เตรียมข้อมูลสำหรับพล็อต: ตั้ง Index เป็น DateTime
                df_to_plot = df_to_plot.set_index('DateTime')

                if not df_to_plot.empty:
                     # พล็อตกราฟ
                     st.line_chart(df_to_plot['Total import kW demand'])
                     # แสดงคำอธิบายใต้กราฟ
                     st.caption(f"กราฟแสดงค่า kW demand สำหรับช่วงเวลา: {graph_period_label} (ข้อมูลดิบ ไม่รวมจำลอง EV)")
                else:
                     st.warning(f"ไม่พบข้อมูลที่ถูกต้องสำหรับสร้างกราฟในช่วงเวลาที่เลือก ({graph_period_label}) หลังการตั้งค่า Index")
            else:
                 # แจ้งเตือนหากไม่มีข้อมูลในช่วงที่เลือก
                 st.warning(f"ไม่พบข้อมูลสำหรับสร้างกราฟในช่วงเวลาที่เลือก: {graph_period_label}")

        except KeyError as ke:
             st.error(f"เกิดข้อผิดพลาดในการเข้าถึงข้อมูลสำหรับกราฟ: ไม่พบคอลัมน์ {ke}")
