...
HOLIDAYS_TOU_2025_STR = ["2025-01-01","2025-01-04","2025-01-05","2025-01-11","2025-01-12","2025-01-18","2025-01-19",
                         "2025-01-25","2025-01-26","2025-02-01","2025-02-02","2025-02-08","2025-02-09","2025-02-12", # Makha Bucha
                         ...
                         "2025-04-05",
                         #"2025-04-06", # Chakri Day (Sunday)
                         "2025-04-07", # Substitution Chakri Day
                         "2025-04-12",
                         "2025-04-13", # Songkran
                         "2025-04-14", # Songkran
                         "2025-04-15", # Songkran
                         "2025-04-19",
                         "2025-04-20","2025-04-26","2025-04-27","2025-05-01", # Labour Day
                         "2025-05-03","2025-05-04",
                         "2025-05-05", # Coronation Day
                         "2025-05-10", # Visakha Bucha Day (Sat - tentative)
                         "2025-05-11",
                         "2025-05-12", # Substitution Visakha Bucha (Mon - tentative)
                         "2025-05-17","2025-05-18","2025-05-24","2025-05-25","2025-05-31",
                         "2025-06-01",
                         "2025-06-03", # Queen's Birthday
                         "2025-06-07","2025-06-08","2025-06-14","2025-06-15","2025-06-21","2025-06-22",
                         "2025-06-28","2025-06-29","2025-07-05","2025-07-06",
                         "2025-07-10", # Asanha Bucha (Thu - tentative)
                         "2025-07-11", # Buddhist Lent (Fri - tentative)
                         "2025-07-12",
                         "2025-07-13","2025-07-19","2025-07-20","2025-07-26","2025-07-27",
                         ...
                         "2025-10-25","2025-10-26","2025-11-01",
...
if st.session_state.full_dataframe is not None and not st.session_state.full_dataframe.empty:
    ...
    # Checkbox for showing the graph
    if st.checkbox("📊 แสดงกราฟ Load Profile (ข้อมูลดิบจากไฟล์)", key="show_graph"):
        st.subheader("กราฟ Load Profile (kW Demand)")
        try:
            # ตรวจสอบว่า DataFrame ไม่ว่างเปล่าก่อนพล็อต
            if not st.session_state.full_dataframe.empty:
                # เตรียมข้อมูลสำหรับ st.line_chart: ตั้งค่า Index เป็น DateTime
                # ใช้ข้อมูลจาก st.session_state.full_dataframe ซึ่งเป็นข้อมูลดิบที่ประมวลผลแล้ว
                # ก่อนที่จะมีการกรองตาม ปี/เดือน หรือการเพิ่มโหลด EV
                df_for_plot = st.session_state.full_dataframe.set_index('DateTime')

                # ตรวจสอบอีกครั้งหลัง set_index เผื่อมีปัญหา
                if not df_for_plot.empty:
                     # เลือกเฉพาะคอลัมน์ 'Total import kW demand' มาพล็อต
                     st.line_chart(df_for_plot['Total import kW demand'])
                     st.caption("กราฟแสดงค่า kW demand ตลอดช่วงเวลาในไฟล์ข้อมูลที่อัปโหลด (ก่อนการกรองหรือจำลอง EV)")
                else:
                     st.warning("ไม่พบข้อมูลสำหรับสร้างกราฟ (อาจมีปัญหาหลังการตั้งค่า Index)")
            else:
                 st.warning("DataFrame ว่างเปล่า ไม่สามารถสร้างกราฟได้")

        except KeyError:
             st.error("เกิดข้อผิดพลาด: ไม่พบคอลัมน์ 'DateTime' หรือ 'Total import kW demand' สำหรับสร้างกราฟ")
        except Exception as plot_ex:
            st.error(f"ไม่สามารถสร้างกราฟได้: {plot_ex}")
            st.error(f"Traceback: {traceback.format_exc()}") # แสดง Traceback เพิ่มเติมหากต้องการ Debug
